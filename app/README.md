...
| `app.cli` | `chat_loop()`：交互式多轮对话，每轮只调 `loop.turn(user_input)`（含 `/exit` `/help` `/history` `/switch` `/new`）；`install_approver()` / `cli_approver()`：装 `tools/approve` 审批者，`run_command` 由人看着完整 argv 与 cwd 放行 / 拒绝 |
...
[`tests/test_cli_approval.py`](../tests/test_cli_approval.py)，不在本族内单放测试文件。
