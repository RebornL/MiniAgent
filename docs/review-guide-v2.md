...
4. **【已闭合，#18】审批者的死路**：审批者安装已归装配点（`assembly.open_harness(approver=...)`，`resume_session` 原样透传；`None` = 不装 = 默认拒绝），CLI 经 `chat_loop` 显式传 `cli_approver()`。恢复出的会话从此可执行已批准命令，deny-on-resume 升格为契约断言（`tests/test_app.py`）。直接用 `build_harness` 的调用方仍不装审批者（`ask` → 无裁决者 → 默认拒绝）——安全侧不变。
