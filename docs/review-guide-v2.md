...
1. **§9 已知偏差仍在**：`capabilities/*/definition/` 的 7 个包里 **6 个仍是 T1 整体搬移**（类型与实现没分家）——`persistence` 492 行、`validation` 240、`compaction` 214、`skills` 143、`retry` 108、`tracing` 97，都是**实现住在被当作契约的包里**。而包化规则之后新生的能力（如 `shell/definition`，59 行）定义得干净——这本身就是「规则有效、旧包欠拆」的对照。
   实测依据：T2 / T3 / T4 三次改存储内部（水位、尾行判定、写后缓冲）**每次都不得不动 `capabilities/persistence/definition/`**，正是 `docs/packaging.md` §1 第 3 条要避免的情形。
   **已另开票 [\#14](https://github.com/RebornL/MiniAgent/issues/14)，尚未开工。**规格 #2 的 user story 18「包按变化速率拆分」目前只做到**层级分离**。
